...
def count_first_digits(data_list):
    ...
    return data_count, data_pct, total_count, total_Votes
...
def bar_chart(data_pct):
    ...
    ax.set_title('Rodolfo Votes vs. Benford Values', fontsize=15)
    ax.set_ylabel('Frequency (%)', fontsize=16)
    ax.set_xticks(index)
    ax.set_xticklabels(index, fontsize=14)

    # build bars    
    rects = ax.bar(index, data_pct, width=0.95, color='black', label='Petro Votes')
...
def main():
    ...
    while True:
        ...
        try:
            ...
            break
    data_count, data_pct, total_count, total_Votes = count_first_digits(data_list)
    expected_counts = get_expected_counts(total_count)
    print("\nobserved counts = {}".format(data_count))
    print("expected counts = {}".format(expected_counts), "\n")
    print("Total Votes at National Level = {:,}".format(total_Votes), "\n")
